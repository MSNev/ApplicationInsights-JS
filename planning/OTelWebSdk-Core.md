...
export interface IOTelClientSDK {
  ...
  readonly config: IConfigurationHandler;
...
export function createOTelClientSDK(config: IOTelClientSDKConfig = {}): IOTelClientSDK {
  ...
  // Create configuration handler for dynamic config
  const _configHandler = createConfigurationHandler(_config);
  
  // Create instrumentation manager
  const _instrumentationManager = createInstrumentationManager();
  
  // Setup core components with the configuration handler
  const _contextManager = createContextManager(_configHandler);
  const _traceProvider = createTraceProvider(_configHandler, _contextManager);
  const _logProvider = createLogProvider(_configHandler, _contextManager);
  const _meterProvider = createMeterProvider(_configHandler, _contextManager);
  ...
  const _self: _IOTelClientSDKInternal = {
    ...
    get config() { return _configHandler; },
...
```typescript
/**
 * Base interface for configuration handlers
 */
export interface IConfigurationHandler {
  ...
  onConfigChange(listener: (key: string, value: any) => void): void;
  /** Remove a configuration change listener */
  offConfigChange(listener: (key: string, value: any) => void): void;
}

/**
 * Create a new configuration handler
 * @param config Initial configuration values
 * @returns A new configuration handler instance
 */
export function createConfigurationHandler(config: Record<string, any> = {}): IConfigurationHandler {
  // Private closure variables
  const _config = { ...config };
  const _listeners: Array<(key: string, value: any) => void> = [];
  
  // Create the instance with direct method implementations
  const _self: IConfigurationHandler = {
    // Get a configuration value
    getConfig<T>(key: string, defaultValue?: T): T {
      if (key in _config) {
   ...
   */
  function _notifyListeners(key: string, value: any): void {
    for (let i = 0; i < _listeners.length; i++) {
      try {
        _listeners[i](key, value);
      } catch (e) {
        console.error("Error in configuration change listener", e);
      }
    }
  }
...
export interface IInstrumentation {
  ...
  isEnabled(): boolean;
  /** Update the instrumentation with new configuration */
  update(config: IConfigurationHandler): void;
}
...
export abstract class BaseInstrumentation implements IInstrumentation {
  ...
  private _config: IConfigurationHandler;
   ...
   */
  constructor(name: string, version: string, config: IConfigurationHandler) {
    this.name = name;
    ...
    dynamicProto(BaseInstrumentation, this, (_self) => {
      ...
      _self.update = (newConfig: IConfigurationHandler) => {
   ...
   * @protected
   */
  protected abstract _onUpdate(config: IConfigurationHandler): void;
}
...
class HttpInstrumentation extends BaseInstrumentation {
  ...
  constructor(config: IConfigurationHandler) {
  ...
  protected _onUpdate(config: IConfigurationHandler): void {
    // React to configuration changes
    const captureHeaders = config.getConfig<boolean>('http.captureHeaders', false);
